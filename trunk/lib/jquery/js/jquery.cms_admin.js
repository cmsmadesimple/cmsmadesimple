        ...
        cms_helper.cms_helpDialog = function() {
            ...
            $('.cms_help img.cms_helpicon').on('click', function() {
                ...
                if (key.length && $('#cmshelp_' + key).length === 0) {
                    ...
                    // put it in the div.
                    var e = $('<div class="cms_helptext" title="' + cms_data.title_help + ': ' + title + '" id="cmshelp_' + key + '" style="display: none;"></div>');
...
//  else { title = title.replace(re, '<br>'); } //TODO title element is populated by .text() which entitizes < > etc
...
function cms_confirm(msg, title, yestxt, notxt) {
    ...
    var re = /(?:\\\\r\\\\n|\\r\\n|\\\\n|\\n|\\\\r|\\r)/g;
    msg = msg.replace(re, '<br>');
    if ( !title) { title = cms_lang('confirm'); }
//  else { title = title.replace(re, '<br>'); } //TODO title element is populated by .text() which entitizes < > etc
    if ( !yestxt) yestxt = cms_data.lang_yes;
    if ( !notxt ) notxt = cms_data.lang_no;
