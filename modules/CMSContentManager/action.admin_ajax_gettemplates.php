...
try {
  ...
  if( $design_id > 0 )
  {
    ...
    case 'alldesign':
      ...
      $out = array();
      foreach( (array) $templates as $one )
      {
        if( !$one->get_listable() ) continue;
        $out[$one->get_id()] = $one->get_name();
      }
    ...
    case 'designpage':
      ...
      if( !count($out) ) $out = $get_all_page_templates();
      break;

    case 'allpage':
      $out = $get_all_page_templates();
      break;
    }
  }
}
