...
const FILTER_KEY = 'PressRoom-defaultadmin-filter';
...
if( $articles ) {
    foreach( $articles as $article ){
        ...
        if( $uid == $article->author_id && $this->CheckPermission( PressRoom::OWN_PERM) ) {
            $metadata[$aid]['owner'] = true;
        }
        if( $this->CheckPermission( PressRoom::APPROVE_PERM ) ) {
...
if( $this->CheckPermission( PressRoom::MANAGE_PERM ) ) {
    ...
    if( $this->CheckPermission( PressRoom::DELOWN_PERM ) ) $bulk_list['del'] = $this->Lang('delete');
    if( $this->CheckPermission( PressRoom::APPROVE_PERM ) ) $bulk_list['status_approve'] = $this->Lang('set_status_approve');
} else if( $this->CheckPermission( PressRoom::APPROVE_PERM ) ) {
