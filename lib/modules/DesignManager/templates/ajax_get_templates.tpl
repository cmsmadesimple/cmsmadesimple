...
<div class="row">
  <div class="pageoptions options-menu half">
    ...
    {/if}
    <a id="edittplfilter" accesskey="f" title="{$mod->Lang('prompt_editfilter')}">{admin_icon icon='filter' alt=$mod->Lang('prompt_editfilter')}&nbsp;{$mod->Lang('filter')}</a>&nbsp;&nbsp;
    {if $have_locks}
...
{if isset($templates)}
  <table class="pagetable" id="templatetable">
	...
	<tr class="{$rowclass}">
	  ...
	  {* default column *}
	  <td class="col_dflt">
	    {if !empty($list_all_types[$type_id])}
