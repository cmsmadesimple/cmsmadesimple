(function($){
	...
	_create: function() {
	    ...
	    if( v < 1 || v > 3600 ) throw 'autoRefresh interval must be between 1 and 100';
	...
	_setOption: function( key, val ) {
	    if( key == 'url' ) {
		if( typeof val === 'string' ) this.options.url = val;
		this.start();
		this.refresh();
	    } else if( key == 'DATA' ) {
	...
	refresh: function() {
		...
		cms_busy(false);
		if( self.options.done_handler ) self.options.done_handler();
	    });
	}
    });
})(jQuery);
