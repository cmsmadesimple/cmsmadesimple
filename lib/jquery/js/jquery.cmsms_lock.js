...
(function($){
  $.widget('cmsms.lockManager', {
	...
	if( self.options.lock_timeout ) {
	    ...
	    console.log('creating initial lock');
	...
	if( self._settings.locked && self._settings.lock_id > 0 ) {
		...
		if( xhr.status != 200 ) return $.Deferred().reject(data,textStatus);
		if( textStatus != 'success' ) return $.Deferred().reject(data,textStatus);
  		if( data.status == 'error' )  return $.Deferred().reject(data,textStatus);
	    }).done(function(data,textStatus){
		console.debug('unlock succeeded');
